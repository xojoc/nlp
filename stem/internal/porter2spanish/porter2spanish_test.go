...
import (
	...
	"gitlab.com/gonlp/stem/porter2spanish"
...
func TestStemBytes(t *testing.T) {
	...
	for scanner.Scan() {
		...
		s := porter2spanish.StemString(words[0])
		if s != words[1] {
...
func loadWords() {
	...
	f := util.MustOpen("vocabulary.txt")
	defer f.Close()
	scanner := bufio.NewScanner(f)
	for scanner.Scan() {
